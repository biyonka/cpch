...
setwd("~/Documents/Research/cpch/multiple_pch")
...
#rename "Max-P" to say "Standard(Max-P)"
final_df$comb_method = sapply(final_df$comb_method, function(x){gsub("Max-P", 'Standard/Max-P', x)} )
adapt = ggplot(final_df, aes(x = theta, y = value, linetype = mt_method, color= comb_method)) + geom_line() + 
...
#set directory path to where you want plot to be saved
ggsave(filename = "adapt_plot.png",
       plot = adapt, path = 'Plots', bg = 'white',
       height = 7, width = 11)