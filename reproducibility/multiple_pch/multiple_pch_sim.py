...
def sim_study_general_n_r(pi1, mus, p, m, r, M, alpha, power_iters, N, tau = 0.1, save_XX = False):
    ...
    cpch_b_bh_fdr, cpch_b_storey_fdr, b_bh_fdr, b_storey_fdr, b_dhh_fdr, b_sdhh_fdr = [], [], [], [], [],[]
    ...
    cpch_b_bh_power, cpch_b_storey_power, b_bh_power, b_storey_power, b_dhh_power, b_sdhh_power = [],[],[],[],[],[]
    ...
    for i in np.arange(power_iters):
      ...
      ##Bonferroni-Storey
        ...
        b_storey_decisions = storey(bon_pvals, alpha)
        ...
        if len(shortened_bon) >= 1:
            ##Simes-DHH
            b_dhh_decisions = mt.multipletests(shortened_bon, alpha, method = 'fdr_bh')[0]
            shortened_null_bool_b = null_bool[bon_pvals <= tau]
            ##Simes-sDHH
            b_sdhh_decisions = storey(shortened_bon, alpha)

            b_dhh_fdr.append(sum(b_dhh_decisions[shortened_null_bool_b])/max(1, sum(b_dhh_decisions)))
            b_sdhh_fdr.append(sum(b_sdhh_decisions[shortened_null_bool_b])/max(1, sum(b_sdhh_decisions)))
            b_dhh_power.append(sum(b_dhh_decisions[np.logical_not(shortened_null_bool_b)])/sum(1-null_bool))
            b_sdhh_power.append(sum(b_sdhh_decisions[np.logical_not(shortened_null_bool_b)])/sum(1-null_bool))
        else: #none of the p-values were less than tau, so dont reject anything
            b_dhh_fdr.append(0)
            b_sdhh_fdr.append(0)
            b_dhh_power.append(0)
            b_sdhh_power.append(0)

#             f_dhh_decisions = np.repeat(0, len(fisher_pvals))
#             f_sdhh_decisions = np.repeat(0, len(fisher_pvals))
#             shortened_null_bool_f =  null_bool #doesn't matter since it's just sucpching up 0's anyways
        ...
        cpch_b_bh_fdr.append(np.sum(cpch_b_bh_decisions[null_bool])/max(1, np.sum(cpch_b_bh_decisions)))
        cpch_b_storey_fdr.append(np.sum(cpch_b_storey_decisions[null_bool])/max(1, np.sum(cpch_b_storey_decisions)))
        b_bh_fdr.append(np.sum(b_bh_decisions[null_bool])/max(1, np.sum(b_bh_decisions)))
        b_storey_fdr.append(np.sum(b_storey_decisions[null_bool])/max(1, np.sum(b_storey_decisions)))
        ...
        cpch_b_bh_power.append(np.mean(cpch_b_bh_decisions[alts]))
        cpch_b_storey_power.append(np.mean(cpch_b_storey_decisions[alts]))
        b_bh_power.append(np.mean(b_bh_decisions[alts]))
        b_storey_power.append(np.mean(b_storey_decisions[alts]))

        ada_power.append(np.mean(ada_decisions[alts]))
    #if num_alt > 0:
    return np.array([cpch_f_bh_fdr, cpch_s_bh_fdr, cpch_b_bh_fdr,
                     cpch_f_storey_fdr, cpch_s_storey_fdr, cpch_b_storey_fdr,
                     f_bh_fdr, s_bh_fdr, b_bh_fdr,
                     f_storey_fdr, s_storey_fdr, b_storey_fdr,
                     f_dhh_fdr, s_dhh_fdr, b_dhh_fdr,
                     f_sdhh_fdr, s_sdhh_fdr, b_sdhh_fdr,
                     ada_fdr,
                    cpch_f_bh_power, cpch_s_bh_power, cpch_b_bh_power,
                    cpch_f_storey_power, cpch_s_storey_power, cpch_b_storey_power,
                    f_bh_power, s_bh_power, b_bh_power,
                    f_storey_power, s_storey_power, b_storey_power,
                    f_dhh_power, s_dhh_power, b_dhh_power,
                    f_sdhh_power, s_sdhh_power, b_sdhh_power,
                     ada_power])#
...
'cpch_f_bh_fdr', 'cpch_s_bh_fdr', 'cpch_b_bh_fdr',
                     'cpch_f_storey_fdr', 'cpch_s_storey_fdr', 'cpch_b_storey_fdr',
                     'f_bh_fdr', 's_bh_fdr', 'b_bh_fdr',
                     'f_storey_fdr', 's_storey_fdr', 'b_storey_fdr',
                     'f_dhh_fdr', 's_dhh_fdr', 'b_dhh_fdr',
