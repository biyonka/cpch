...
setwd("~/Documents/Research/cpch/single_pc_power/Data_dependence/") 
...
df = df[df$rho %in% c(0.1, 0.3, 0.5),]

correction_method = rep(c(rep('Standard', 2),rep('cPCH', 2)), nrow(df)/4)
comb_method = rep(c('Fisher', 'Simes'), nrow(df)/2)

df['correction_method'] = correction_method
df['correction_method'] = factor(correction_method, levels = c('Standard','cPCH'))
df['comb_method'] = comb_method
#sort df columns so cpch methods are
df = df[order(match(df$correction_method, c('Standard','cPCH'))),]
...
#labelling_2 = function(string){paste0('rho =', string)}
#df_null$rho = sapply(df_null$rho, labelling_2)
#df_alt$rho = sapply(df_alt$rho, labelling_2)
#df$rho = sapply(df$rho, labelling_2)
...
single_pc_alt = ggplot(data=df_alt, aes(x=sig_strength, y=p_reject, color = correction_method, linetype = comb_method)) +
 ...
 # theme(strip.background =element_rect(fill="lightgray")) + 
  facet_wrap(~r + k+rho, nrow = 3, dir = 'v', labeller = custom_labeller) +
  theme(panel.border = element_blank()) + 
  theme(aspect.ratio = 1, strip.text.x = element_text(size = 15),legend.position="none",
...
single_pc_null = ggplot(data=df_null, aes(x=sig_strength, y=p_reject, color = correction_method, linetype = comb_method)) +
  ...
  theme(aspect.ratio = 1, 
    strip.text.x = element_text(size = 17), legend.position="none",
...
ggsave(filename = paste0("dependence_single_pc_alt_plot_m_", m,".eps"),
       plot = single_pc_alt, path = '~/Documents/Research/cpch/Plots/', bg = 'white',
       height = 7, width = 7)

#change directory to where you want plot to be saved
ggsave(filename = paste0("dependence_single_pc_null_plot_m_", m,".eps"),
