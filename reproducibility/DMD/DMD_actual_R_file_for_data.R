...
# 
new_dmd = read.csv('~/Documents/Research/cpch/Data/dmd_followup.csv')
bhqvals_cpch = p.adjust(new_dmd$pvalue.3, method = 'BH', n = 1871)
rejected_bh_cpch = 1*(bhqvals_cpch <= q) #1*(storey(new_dmd$pvalue.1, q)$significant) #
...
cpch_pvals_dmd = read.csv('~/Documents/Research/cpch/Data/cpch_simes_pvals_dmd_followup_screen_3.csv')#
# #read in cpch pvals data frame
fisher_pvals_dmd = read.csv('~/Documents/Research/cpch/Data/simes_pvals_dmd_followup_screen_3.csv')#('~/Documents/Research/cpch/Data/fisher_pvals_dmd_followup.csv')
...
fisher_adapt_3_3 <- adapt_gmm(x=x, pvals=fisher_pvals_dmd$simes_pvals_3_3, alphas=alphas,
                              beta_formulas = formulas, model_type = "neural", nclasses= c(5))
rejected_adapt_fisher_3_3 = 1*(fisher_adapt_3_3$qvals <= q)
sum(rejected_adapt_fisher_3_3)
#adapt fisher classical, r=2
fisher_adapt_3_2 <- adapt_gmm(x=x,pvals=fisher_pvals_dmd$simes_pvals_3_2, alphas=alphas,
                              beta_formulas = formulas, model_type = "neural", nclasses= c(5))
...
#adafilter
rejections_ada_3_2 = adaFilter(as.matrix(dmd[,c('pvalue.1', 'pvalue.4', 'pvalue.2')]), r=2, alpha = q)$decision
sum(rejections_ada_3_2)
rejections_ada_3_3 = adaFilter(as.matrix(dmd[,c('pvalue.1', 'pvalue.4', 'pvalue.2')]), r=3, alpha = q)$decision
...
if (length(dhh_pvals_3_2) >= 1){
  ...
  ada_dhh_3_2 <- adapt_gmm(x=dhh_x_3_2, pvals=dhh_pvals_3_2, alphas=alphas,
                           beta_formulas = formulas, model_type = "neural", nclasses= c(4))
...
cat(paste0('2 & ',sum(rejected_adapt_cpch_3_2), sum(rejections_ada_3_2),' & ',' & ',sum(rejected_adapt_dhh_3_2),' & ', sum(rejected_adapt_fisher_3_2),'\\
3 & ',sum(rejected_adapt_cpch_3_3),' & ',sum(rejections_ada_3_3),' & ',sum(rejected_adapt_dhh_3_3),' & ',sum(rejected_adapt_fisher_3_3),'\\'))
