...
run_sim = function(theta){
  ...
  z = data.frame(z1, z2)
  cpch = apply(z, 1, calc_cpch_pvals, true_mu = c(0, 0), oracle = F)
  ...
  return(c(tpr_adaf, tpr_hdmt, 
           ...
           tpr_storey_c, tpr_storey_cpch, tpr_storey_dact, tpr_dhh_s,
           fdr_adaf, fdr_hdmt,
           fdr_adapt_c, fdr_adapt_cpch, fdr_adapt_dhh, fdr_adapt_dact,
           fdr_bh_c, fdr_bh_cpch, fdr_bh_dact, fdr_dhh_bh,
           fdr_storey_c, fdr_storey_cpch, fdr_storey_dact, fdr_dhh_s))
}
...
df = data.frame('it' = iteration, 'theta' =rep(thetas, each = length(output[,1])), 
                'comb_method' = rep(c('AdaFilter', 'HDMT', 'Max-P', 'cPCH', 'DHH', 'DACT',
                                      rep(c('Max-P', 'cPCH', 'DACT', 'DHH'), 2)), 8),
                'mt_method' = rep(c(c('AdaFilter', 'HDMT'), rep('AdaPT-GMM', 4), rep('BH', 4), rep('Storey', 4)), 8),
                'metric' = rep(c(rep('TPR', 14), rep('FDR', 14)), 4),
