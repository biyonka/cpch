...
parser.add_argument('rho', metavar='rho', type=float,
                    help='the covariance between test statistics')
...
def single_pc_power(r, k, rho, alpha, sig_strength, replicates, N, tau = 0.1, save_XX = False):
    ...
    XX = np.random.multivariate_normal(mu_vec, cov, replicates)
   ...
   # np.std(cpch_pvals_f_oracle <= alpha, ddof = 1), np.std(cpch_pvals_s_oracle <= alpha, ddof=1), np.std(cpch_pvals_b_oracle <= alpha, ddof = 1)
    ...
    methods = np.array(['Fisher', 'Simes', 'cPCH-Fisher', 'cPCH-Simes'])

    data_df = pd.DataFrame({'m': np.repeat(m, 4), 'r': np.repeat(r, 4), 'k': np.repeat(k, 4),
                            'rho': np.repeat(rho, 4),'sig_strength': np.repeat(sig_strength, 4),
                        'methods': methods, 'p_reject': p_reject, 'se': ses})
    return data_df

data_df = single_pc_power(r, k, rho, alpha, sig_strength, replicates, N, tau = 0.1, save_XX = False)
#change path to where and how you want the data to be saved
csv_name = "Data_dependence/m_" + str(m) + "/m_" + str(m) + "_r_" + str(r) + '_k_' + str(k) + '_ss_' + str(sig_strength) +'_rho_' + str(rho)+'.csv'
